...
<link rel="stylesheet" href="/css/boardupload.css?a">
<script>
...
<div class="post_upload">
   ...
   <input type="hidden" name="post_num" id="post_num" value="${viewPost.post_num}" />
   <input type="hidden" id="boardNum" name="boardNum" value="B0000001" />
      ...
      <%-- <c:forEach var="data" items="${ file}" varStatus="num">   
      <input type="image" src="${data}" value="${num.count}" name="img_view" id="img_view1" class="image-upload-wrap" /><br/>   
      </c:forEach> --%>
      ...
      <div id="data_file_txt" style="margin:40px;">
      ...
      </div><input type="text" name="text_content" id="text_content" value="${viewPost.post_content}" placeholder="사진(이미지) 제목 또는 이름을 입력해주세요">   
        <!-- <i class="fas fa-camera" id="btn_add"></i> -->
        <div>
        <input type="button" value="추가" id="btn_add"/>
        <input type="button" id="btn_cancel" value="취소">        
         </div>
      <!-- input type="hidden" id="h_user_id" value="hyde940"/> -->            
    </form>
    </div>
    </div>
    </div>
